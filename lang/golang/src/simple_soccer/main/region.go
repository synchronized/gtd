...
func NewRegion(left, top, right, bottom float64, id int) *Region {
	...
	r.vCenter = Vector2d{(left + right) * 0.5, (top + bottom) * 0.5}
...
func (r *Region) GetRandomPosition() Vector2d {
	return Vector2d{rand.Float64()*(r.dRight-r.dLeft) + r.dLeft,
		rand.Float64()*(r.dBottom-r.dTop) + r.dTop}
}

func (r *Region) Inside(pos Vector2d, rtype RegionType) bool {
