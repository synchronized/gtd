...
class Hen{
public:
    static const int func1() { return 0; }
    const static int func2() { return 0; }
    const int func3() const { return 0; }
};

const int func1() { return 0; }
const int func2() { return 0; }
// error: 非成员函数‘int func3()’不能拥有 cv 限定符
// int func3() const { return 0; }
...
int main(){
	...
	*n = 30;
	...
	const int (*o)() = func1;
	o = func2;
	cout << "const int (*o)():" << reinterpret_cast<long>(o) << endl;

	const int (* const p)() = func1;
	//p = func2;  //error can't write to readonly var "p"
	cout << "const int (*p)():" << reinterpret_cast<long>(p) << endl;

	const int (* q) () = Hen::func1;
	q = Hen::func2;
	cout << "const int (*q)():" << reinterpret_cast<long>(q) << endl;
