...
function mypow(x, y)
...

    {
        ...
        lua_getglobal(lState, "mypow");
        if (!lua_isfunction(lState, -1)) {
            printf("lua func mypow not found or not function\n");
            return -1;
        }
        lua_pushinteger(lState, 3);
        lua_pushinteger(lState, 4);
        if (lua_pcall(lState, 2, 1, 0)) {
            if (lua_isstring(lState, -1)) {
                printf("call lua func mypow failed err:%s\n", lua_tostring(lState, -1));
            } else {
                printf("call lua func mypow failed err:%s\n", "unkonw");
            }
            return -1;
        ...
        if (!lua_isnumber(lState, -1)) {
            printf("call lua func mypow failed err: invalid return value type:%s\n", lua_type(lState, -1));
            return -1;
        }
        printf("call lua func mypow success result: %f\n", lua_tonumber(lState, -1));
    }
