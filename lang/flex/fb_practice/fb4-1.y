...
%token NUMBER
%token ON
%token ONDUPLICATE /*由于首先的向前查看而是用的权益之计*/
%token OPTIMIZE
%token OPTION
%token OPTIONALLY
%token ORDER
...
expr: expr IN '(' val_list ')'	{ emit("ISIN %d", $4); }
	...
	/* 常规函数 */
expr: NAME '(' opt_val_list ')'	{ emit("CALL %d %s", $3, $1); }
	;
...
column_list: NAME	{ emit("COLUMN %s", $1); free($1); $$ = 1; }
	| column_list ',' NAME { emit("COLUMN %s", $3); free($3); $$ = $$ + 1; }
	;

select_opts: { $$ = 0; }
	| select_opts ALL
		{ if($1 & 01) lyyerror(@2, "duplicate ALL option"); $$ = $1 | 01; }
	| select_opts DISTINCT
		{ if($1 & 02) lyyerror(@2, "duplicate DISTINCT option"); $$ = $1 | 02; }
	| select_opts DISTINCTROW
		{ if($1 & 04) lyyerror(@2, "duplicate DISTINCTROW option"); $$ = $1 | 04; }
	| select_opts HIGH_PRIORITY 
		{ if($1 & 010) lyyerror(@2, "duplicate HIGH_PRIORITY option"); $$ = $1 | 010; }
	| select_opts STRAIGHT_JOIN 
		{ if($1 & 020) lyyerror(@2, "duplicate STRAIGHT_JOIN option"); $$ = $1 | 020; }
	| select_opts SQL_SMALL_RESULT
		{ if($1 & 040) lyyerror(@2, "duplicate SQL_SMALL_RESULT option"); $$ = $1 | 040; }
	| select_opts SQL_BIG_RESULT
		{ if($1 & 0100) lyyerror(@2, "duplicate SQL_BIG_RESULT option"); $$ = $1 | 0100; }
	| select_opts SQL_CALL_FOUND_ROWS
		{ if($1 & 0200) lyyerror(@2, "duplicate SQL_CALL_FOUND_ROWS option"); $$ = $1 | 0200; }
	;

select_expr_list: select_expr { $$ = 1; }
	| select_expr_list ',' select_expr	{ $$ = $1 + 1; }
	| '*' { emit("SELECTALL"); $$ = 1; }
	| NAME '.' '*' { emit("SELECTNAMEALL %s", $1); free($1); $$ = 1; }
	;
...
table_factor: NAME opt_as_alias index_hint	{ emit("TABLE %s", $1); free($1); }
	| NAME '.' NAME opt_as_alias index_hint	
		{ emit("TABLE %s.%s", $1, $3); free($1); free($3); }
...
opt_left_or_right_outer: /* 空 */ { $$ = 0; }
	| LEFT opt_outer { $$ = 1 + $2; }
	| RIGHT opt_outer	{ $$ = 2 + $2; }
	;

opt_join_condition:	/* 空 */ 
	| join_condition
	;
...
table_subquery: '(' select_stmt ')' { emit("SUBQAUERY"); }
	;
...
create_definition: /* 空 */ { emit("STARTCOL"); }
	| NAME data_type column_atts { emit("COLUMNDEF %d %s", $3, $1); free($1); }

column_atts:  /* 空 */ { $$ = 0; }
	| column_atts NOT NULLX 
		{ emit("ATTR NOT NULL"); $$ = $1 + 1; }
	| column_atts NULLX
		{ emit("ATTR NULL"); $$ = $1 + 1; }
	...
	| column_atts UNIQUE KEY 
		{ emit("ATTR UNIQUEKEY"); $$ = $1 + 1; }
	| column_atts KEY 
		{ emit("ATTR PRIKEY"); $$ = $1 + 1; }
...
/*	| INT opt_length opt_uz { $$ = 40000 + $2 + $3; } */
	| INTEGER opt_length opt_uz { $$ = 5000 + $2 + $3; }
