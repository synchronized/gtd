...
static asciictx * ascii_tree_create_context()
{
...
static asciinode * ascii_tree_build_from_avltree_recursive(avltree * t)
{
  asciinode * node;
  ...
  node->left = ascii_tree_build_from_avltree_recursive(t->left);
  node->right = ascii_tree_build_from_avltree_recursive(t->right);
...
static asciinode * ascii_tree_build_from_avltree(avltree * t)
{
  asciinode *node;
  if (t == NULL) return NULL;
  node = ascii_tree_build_from_avltree_recursive(t);
  node->parent_dir = 0;
  return node;
}
...
void ascii_tree_print_from_avltree(avltree * t)
{
  ...
  int xmin, i;
  if (t == NULL) return;
  ctx = ascii_tree_create_context();
  proot = ascii_tree_build_from_avltree(t);
