...
static void print_build_handler(void *t, char *buf, void **left, void **right) {
  struct rb_node *node = (struct rb_node*)t;
  int color_char = rb_is_red(node) ? 'R' : 'B';
  sprintf(buf, "%c:%d", color_char, rb_entry(node, struct my_node, rb_node)->key);
  *left = node->rb_left;
  *right = node->rb_right;
}
...
void main()
{
    ...
    for (i=0; i < ilen; i++)
    {
        ...
        printf("== 添加节点: %d\n", a[i]);
        printf("== 树的详细信息: \n");
        ascii_tree_print(mytree.rb_node, print_build_handler);
        printf("\n");
#endif
